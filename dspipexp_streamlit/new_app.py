...
def show_song_card(song, similarity=None, with_recommendations=False):
    ...
    # Parse audio features if needed
    ...
    # Parse sentiment features if needed
    ...
    with st.container():
        ...
        with col1:
            # Try to get album art or YouTube thumbnail
            img_url = None
            if song.get('youtube_url'):
                img_url = get_youtube_thumbnail(song['youtube_url'])
            
            if not img_url:
                img_url = get_album_cover(song.get('track_name', ''), song.get('track_artist', ''))
                
            if img_url:
                try:
                    st.image(img_url, use_column_width=True)
                except:
                    st.image(get_default_album_art(), use_column_width=True)
            else:
                st.image(get_default_album_art(), use_column_width=True)
        
        with col2:
            # Title and artist with similarity score if provided
            ...
            with meta_cols[0]:
                ...
                st.markdown(f"**Album:** {album if album else 'Unknown Album'}")
            
            with meta_cols[1]:
                genre = song.get('playlist_genre', '')
                st.markdown(f"**Genre:** {genre if genre else 'Unknown'}")
            ...
            with action_cols[2]:
                if audio_features:
                    button_key = f"audio_viz_{song.get('id', 'temp')}"
                    if st.button("📊 Audio Profile", key=button_key):
        ...
        # Audio visualization if requested
        ...
        if viz_key in st.session_state and st.session_state[viz_key]:
            with st.expander("Audio Feature Visualization", expanded=True):
                if audio_features:
                    ...
                    st.info("No audio features available for visualization")
                    st.session_state[viz_key] = False
        
        # Show recommendations if requested
        rec_key = f'show_recs_for_{song.get("id")}'
        if rec_key in st.session_state and st.session_state[rec_key] and not with_recommendations:
        ...
        # If this is a recommendation, show explanation
        if similarity is not None and 'audio_score' in song and 'lyrics_score' in song and 'sentiment_score' in song:
            with st.expander("Why this recommendation?"):
                st.info(explain_recommendation(
                    song.get('audio_score', 0), 
                    song.get('lyrics_score', 0), 
                    song.get('sentiment_score', 0)
                ))
                ...
                cols = st.columns(3)
                with cols[0]:
                    st.metric("Sound Profile", f"{song.get('audio_score', 0):.2f}")
                with cols[1]:
                    st.metric("Lyrics", f"{song.get('lyrics_score', 0):.2f}")
                with cols[2]:
                    st.metric("Emotional Tone", f"{song.get('sentiment_score', 0):.2f}")
